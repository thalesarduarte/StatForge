from fastapi import APIRouter, Depends
from sqlalchemy.orm import Session

from app.api.deps import get_db
from app.api.responses import item_response, list_response
from app.repositories.profile_repository import ProfileRepository
from app.schemas.api import ApiEnvelope, ListEnvelope
from app.schemas.profile import ProfileCreate, ProfileRead

router = APIRouter()


@router.get("/", response_model=ListEnvelope[ProfileRead])
def list_profiles(db: Session = Depends(get_db)) -> dict:
    return list_response(ProfileRepository(db).list_all())


@router.post("/", response_model=ApiEnvelope[ProfileRead])
def create_profile(payload: ProfileCreate, db: Session = Depends(get_db)) -> dict:
    return item_response(ProfileRepository(db).create(payload), message="profile created")
